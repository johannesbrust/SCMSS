...
% Initial line types
types.colors    = ['b' 'r' 'k' 'g' 'k' 'c' 'y']; %'k' 'y'
types.lines     = {'-', '-.', ':','-.','-','-.','-'}; %'-',   '-'
...
%         'm',num2str(ms(j)),'_','q',num2str(qs(j)),'_LMSSM'];
    name_EXPERIMENT = ['EXPERIMENT_LMSS_',...
        'm',num2str(ms(j)),'_','q',num2str(qs(j)),'_DENSE'];
    %name_EXPERIMENT = 'EXPERIMENT_FINAL_COMP_LSR1_LMSSM_SCINF';
    data = load([datapath,name_EXPERIMENT],'ex','numit','t_aver','numf');
    ex(:,j) = data.ex(1:np,indAlg(j));
    numit(:,j) = data.numit(1:np,indAlg(j));
    t_aver(:,j) = data.t_aver(1:np,indAlg(j));
    numf(:,j) = data.numf(1:np,indAlg(j));
...
perf_ext_fnc(ex(:,1:size(indAlg,2)),t_aver(:,1:size(indAlg,2)),mleg(indAlg),1,typesp,...
...
perf_ext_fnc(ex(:,1:size(indAlg,2)),numf(:,1:size(indAlg,2)),mleg(indAlg),1,typesp,...
    leglocation1,XTick,XLim,legFontSize1,markerSize,lineWidths);
...
grid on; grid off;
grid on;
title(['Function Calls'],'Interpreter','latex');
