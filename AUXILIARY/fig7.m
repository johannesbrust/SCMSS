...
%         'm',num2str(ms(j)),'_','q',num2str(qs(j)),'_LMSSM'];    
    ...
    ex(:,j) = data.ex(1:np,1);
    numit(:,j) = data.numit(1:np,1);
    t_aver(:,j) = data.t_aver(1:np,1);
    numf(:,j) = data.numf(1:np,1);
...
grid on; grid off;
grid on;
title(['$\textnormal{Time } (q=',num2str(qs(1)),')$'],'Interpreter','latex');
...
title(['$\textnormal{Function Calls } (q=',num2str(qs(1)),')$'],'Interpreter','latex');
