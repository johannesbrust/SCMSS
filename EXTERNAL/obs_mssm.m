function [sigmaStar,pStar,opt1,opt2,spd_check] = obs_mssm( g, S, Y, delta, gamma,varargin)

% Computes the solution to a trust-region subproblem when the quadratic
% model is defined by limited-memory symmetric rank-one (L-MSSM)
% quasi-Newton matrix.
%
% Derived from
%
% "OBS: MATLAB Solver for L-SR1 Trust-Region Subproblems"
% by Johannes Brust, Jennifer Erway, and Roummel Marcia
%
...
%
%--------------------------------------------------------------------------
% 07/16/20, J.B., Modification to deal with rank deficient Psi
% 02/17/21, J.B., Updates for using accelerations (better starting) point
%                   in Newton's method. Modification of indices when 
%                   computing an orthogonal projection.
% 03/01/21, J.B., Modification to enable efficient updating in the L-SR1
% 06/09/22, J.B., Updates for the MSS matrix
% 06/10/22, J.B., Modified Psi and inputs in the Sherman-Morrison-Woodbury
%                   inverse

%                   formula
show        = 0;     % verbosity flag


if ~((nargin==5) || (nargin==7) || (nargin==8))
...
if (nargin==5)  %inputs were S and Y
  ...
  SS          = S'*S;
  W           = SS\eye(size(SS,1));
  invM        = -(W*(triu(SY)+ triu(SY,1)')*W)-gamma.*W;
...
% Adjustments if Psi_ is not full rank
%tau = sqrt(eps);
if nargin~=8
    PsiPsi_      = Psi_'*Psi_;
end

[Ldl,Dldl,Pldl] = ldl(PsiPsi_,'vector');  %ldl decomposition of PsiPsi' 
...
% Update Psi and PsiPsi
%if nargin~=8
    Psi = Psi_(:,Pldl_fullRank);
% else
%     Sk = S(:,Pldl_fullRank);
%     Yk = Y(:,Pldl_fullRank);
% end

PsiPsi  = PsiPsi_(Pldl_fullRank,Pldl_fullRank); 
...
Psig_       = Psi_'*g;
    ...
    % else
%     P_parallel  = Yk*RU-Sk*(gamma.*RU);
%     Psig        = Yk'*g-Sk'*(gamma.*g);
%     Psig_       = Y'*g-S'*(gamma.*g);
% end
...
if (lambda_min>0) & (norm(a_j./Lambda)<=delta)  
  ...
  %if nargin~=8
    pStar     = ComputeSBySMW(gamma,g,Psig_,Psi_,(invM\Inm),PsiPsi_);
%   else
%     pStar     = ComputeSBySMWSY(gamma,gamma,g,Psig_,S,Y,invM,PsiPsi_);
%   end
...
%       else
%           p = PsiPsi\Psig;
%           pStar = -P_parallel*v(1:sizeD) + (1/(gamma+sigmaStar)).*...
%               (Yk*p-Sk*(gamma.*p)) - (g./(gamma+sigmaStar));
%       end
  end
  ...
  if lambda_min < 0
    ...
    if abs(lambda_min-Lambda(1))<tol 
      ...
      if found == 0
         % Update to use "i+1" in this part and "'" on P_parallel(i+1,1:end)'
        e(i+1) = 1;
    ...
    % (acceleration) of Newton's method
    sigmaHat = max((max(abs(a_j)/delta - Lambda)),0);
...
%   end
  %pStar     = ComputeSBySMW(gamma+sigmaStar,g,Psig,Psi,invM,PsiPsi);
   %pStar     = ComputeSBySMW(gamma+sigmaStar,g,Psig_,Psi_,invM,PsiPsi_);
%   if nargin~=8
    pStar     = ComputeSBySMW(gamma+sigmaStar,g,Psig_,Psi_,(invM\Inm),PsiPsi_);
%   else
%     pStar     = ComputeSBySMWSY(gamma,gamma+sigmaStar,g,Psig_,S,Y,invM,PsiPsi_);
%   end
end
...
%     else
%         p1 = Yk'*pStar-gamma.*(Sk'*pStar);
%         p2 = invM\p1;
%         BpStar = gamma.*pStar + Yk*p2 - Sk*(gamma.*p2);  %uses Equation (4)
%     end
...
%In particular,
% s* = -[ 1/tau*( I - Psi(tau*.invM + PsiPsi)^(-1))Psi') ][ g ].
%
...
function [ pStar ] = ComputeSBySMWSY( gamma, tauStar, g, Psig, S, Y, invM, PsiPsi)
%ComputeSBySWM solves (B+tauStar)sStar = -g for sStar where B is
%an L-SR1 matrix using the Sherman-Morison-Woodbury formula.
%In particular,
